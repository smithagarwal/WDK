classdef DataAnnotationApp < handle
    ...
    methods (Access = private)
        ...
        function plotMatchedMarkers(obj)
            ...
            obj.gatherMatchedMarkersStatistics(obj.debuggingMatchedMarkers);
        ...
        %Added by Smith to gather matched Markers statistics
        function gatherMatchedMarkersStatistics(obj,debugMode)
            if(debugMode)
                f = figure;
                uit = uitable(f,'Data',tabulate(obj.matchedSet.Debug),'Position',[20 100 260 200]);
                uit.ColumnName = {'Color','Count','Percentage'};
                uit.FontSize = 15;
                uit.RowStriping = 'on';
            end
        end
